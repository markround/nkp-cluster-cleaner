...
class ConfigManager:
    ...
    def _parse_config(self, config_data: Dict) -> DeletionCriteria:
        ...
        criteria.protected_cluster_patterns = config_data.get(
            "protected_cluster_patterns", []
        )
        criteria.excluded_namespace_patterns = config_data.get(
    ...
    def save_example_config(self, output_file: str):
        ...
        """
        example_config = {
            "protected_cluster_patterns": ["^production-.*", ".*-prod$", "critical-.*"],
            "excluded_namespace_patterns": ["^default$", ".*-prod$"],
            "extra_labels": [
                {"name": "owner", "description": "Cluster owner identifier"},
                {
                    "name": "cost_centre",
        ...
        for extra_label in self.criteria.extra_labels:
            ...
            if label_value is None:
                ...
                if extra_label.regex:
                    errors.append(
                        f"Label '{extra_label.name}' value '{label_value}' does not match pattern '{extra_label.regex}'"
                    )
                else:
                    errors.append(
                        f"Label '{extra_label.name}' value '{label_value}' is invalid"
                    )

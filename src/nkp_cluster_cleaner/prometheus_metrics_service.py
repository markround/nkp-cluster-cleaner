...
class PrometheusMetricsService:
    ...
    def generate_metrics(self) -> str:
        ...
        try:
            ...
            metrics_lines.extend(
                self._get_trend_metrics(cluster_trends_7d, dashboard_summary)
            )
    ...
    def _get_trend_metrics(
        ...
        trend_direction_value = self._encode_trend_direction(
            trend_summary.get("trend_direction")
        )
    ...
    def _get_compliance_metrics(self, compliance_stats: Dict[str, Any]) -> List[str]:
        ...
        compliance_direction_value = self._encode_compliance_direction(
            compliance_summary.get("compliance_direction")
        )
    ...
    def _get_activity_metrics(self, deletion_activity: Dict[str, Any]) -> List[str]:
        ...
        if "deletion_reasons" in deletion_activity:
            ...
            for reason, count in deletion_activity["deletion_reasons"].items():
                ...
                metrics.append(
                    f'nkp_cluster_cleaner_deletion_reason_count{{reason="{safe_reason}"}} {count}'
                )
            metrics.append("")
    ...
    def _get_namespace_metrics(self, namespace_activity: Dict[str, Any]) -> List[str]:
        ...
        if "top_namespaces" in namespace_activity:
            ...
            for namespace, stats in namespace_activity["top_namespaces"].items():
                ...
                metrics.append(
                    f'nkp_cluster_cleaner_namespace_clusters{{namespace="{safe_namespace}",type="total"}} {stats.get("total_clusters", 0)}'
                )
                metrics.append(
                    f'nkp_cluster_cleaner_namespace_clusters{{namespace="{safe_namespace}",type="deletion"}} {stats.get("deletion_clusters", 0)}'
                )
                metrics.append(
                    f'nkp_cluster_cleaner_namespace_clusters{{namespace="{safe_namespace}",type="protected"}} {stats.get("protected_clusters", 0)}'
                )
            metrics.append("")
    ...
    def _get_owner_metrics(self, owner_distribution: Dict[str, Any]) -> List[str]:
        ...
        if "top_owners" in owner_distribution:
            ...
            for owner, stats in owner_distribution["top_owners"].items():
                ...
                metrics.append(
                    f'nkp_cluster_cleaner_owner_clusters{{owner="{safe_owner}",type="total"}} {stats.get("total_clusters", 0)}'
                )
                metrics.append(
                    f'nkp_cluster_cleaner_owner_clusters{{owner="{safe_owner}",type="deletion"}} {stats.get("deletion_clusters", 0)}'
                )
                metrics.append(
                    f'nkp_cluster_cleaner_owner_clusters{{owner="{safe_owner}",type="protected"}} {stats.get("protected_clusters", 0)}'
                )
            metrics.append("")
    ...
    def _get_expiration_metrics(self, expiration_analysis: Dict[str, Any]) -> List[str]:
        ...
        if "current_distribution" in expiration_analysis:
            ...
            for bucket, count in expiration_analysis["current_distribution"].items():
                ...
                metrics.append(
                    f'nkp_cluster_cleaner_expiration_buckets{{timeframe="{safe_bucket}"}} {count}'
                )
            metrics.append("")
    ...
    def _get_infrastructure_metrics(self, database_stats: Dict[str, Any]) -> List[str]:
        ...
        if database_stats.get("redis_memory_used"):
            memory_used_bytes = self._parse_memory_value(
                database_stats["redis_memory_used"]
            )
        ...
        if database_stats.get("redis_memory_peak"):
            memory_peak_bytes = self._parse_memory_value(
                database_stats["redis_memory_peak"]
            )
        ...
        if database_stats.get("earliest_snapshot"):
            try:
                earliest_timestamp = datetime.fromisoformat(
                    database_stats["earliest_snapshot"].replace("Z", "")
                ).timestamp()
                metrics.extend(
                    [
        ...
        if database_stats.get("latest_snapshot"):
            try:
                latest_timestamp = datetime.fromisoformat(
                    database_stats["latest_snapshot"].replace("Z", "")
                ).timestamp()
                metrics.extend(
                    [

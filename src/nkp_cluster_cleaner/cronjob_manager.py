...
class CronJobManager:
    ...
    def get_nkp_cronjobs(self, namespace: str = "kommander") -> List[Dict]:
        ...
        try:
            cronjobs = self.batch_v1.list_namespaced_cron_job(
                namespace=namespace, label_selector="app=nkp-cluster-cleaner"
            )
            ...
            for cj in cronjobs.items:
                cronjob_info = {
                    ...
                    "successful_job_history_limit": cj.spec.successful_jobs_history_limit
                    if cj.spec.successful_jobs_history_limit
                    else 3,
                    "failed_job_history_limit": cj.spec.failed_jobs_history_limit
                    if cj.spec.failed_jobs_history_limit
                    else 1,
            ...
            for job in jobs.items:
                ...
                if job.metadata.owner_references:
                    for owner in job.metadata.owner_references:
                        if owner.kind == "CronJob" and owner.name == cronjob_name:
                            job_info = {
    ...
    def _validate_pod_ownership(
        self, pod_name: str, namespace: str, expected_job_name: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                job = self.batch_v1.read_namespaced_job(
                    name=job_owner, namespace=namespace
                )
    ...
    def get_all_scheduled_tasks_summary(self, namespace: str = "kommander") -> Dict:
        ...
        completed_jobs = [
            job for job in all_recent_jobs if job["status"] in ["Succeeded", "Failed"]
        ]
        successful_jobs = [
            job for job in completed_jobs if job["status"] == "Succeeded"
        ]

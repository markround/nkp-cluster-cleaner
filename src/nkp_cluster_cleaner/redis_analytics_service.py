...
class RedisAnalyticsService:
    ...
    def _get_historical_data(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        snapshot_keys = self.redis_client.zrangebyscore(
            "analytics:snapshots:index", cutoff_timestamp, current_timestamp
        )
    ...
    def get_cluster_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        if len(deletion_counts) >= 7:
            ...
            older_avg = (
                sum(deletion_counts[-14:-7]) / 7
                if len(deletion_counts) >= 14
                else recent_avg
            )
        ...
        return {
            ...
            "summary": {
                ...
                "average_daily_deletions": sum(deletion_counts) / len(deletion_counts)
                if deletion_counts
                else 0,
    ...
    def get_deletion_activity(self, days: int = 14) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "most_common_reason": deletion_reasons.most_common(1)[0]
                if deletion_reasons
                else ("None", 0),
    ...
    def get_compliance_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        average_compliance = (
            sum(compliance_trend) / len(compliance_trend) if compliance_trend else 0
        )
        ...
        if len(compliance_trend) >= 7:
            ...
            older_avg = (
                sum(compliance_trend[-14:-7]) / 7
                if len(compliance_trend) >= 14
                else recent_avg
            )
    ...
    def get_namespace_activity(self, days: int = 30) -> Dict[str, Any]:
        ...
        for snapshot in historical_data:
            ...
            if "clusters_by_namespace" in snapshot:
                for namespace, counts in snapshot["clusters_by_namespace"].items():
                    ...
                    stats["total_clusters"] = max(
                        stats["total_clusters"], counts.get("total", 0)
                    )
        ...
        sorted_namespaces = sorted(
            namespace_stats.items(), key=lambda x: x[1]["activity_score"], reverse=True
        )
        ...
        return {
            ...
            "summary": {
                ...
                "most_active_namespace": sorted_namespaces[0][0]
                if sorted_namespaces
                else "N/A",
                "average_clusters_per_namespace": sum(
                    s["total_clusters"] for s in namespace_stats.values()
                )
                / len(namespace_stats)
                if namespace_stats
                else 0,
    ...
    def get_owner_distribution(self, days: int = 30) -> Dict[str, Any]:
        ...
        sorted_owners = sorted(
            owner_stats.items(), key=lambda x: x[1]["total_clusters"], reverse=True
        )
    ...
    def get_expiration_analysis(self, days: int = 30) -> Dict[str, Any]:
        ...
        for snapshot in historical_data:
            ...
            if "expiration_analysis" in snapshot:
                ...
                for expires_value, count in analysis.get(
                    "common_expires_values", {}
                ).items():
        ...
        return {
            ...
            "summary": {
                "most_common_duration": expiration_patterns.most_common(1)[0]
                if expiration_patterns
                else ("N/A", 0),
                "clusters_without_expiration": current_distribution.get(
                    "no_expiration", 0
                ),
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "current_status": {
                    "clusters_for_deletion": trends_7d["summary"][
                ...
                "week_summary": {
                    "average_deletions": trends_7d["summary"][
                        "average_daily_deletions"
                    ],
                    "trend_direction": trends_7d["summary"]["trend_direction"],
                },
                "month_summary": {
                    "average_deletions": trends_30d["summary"][
                        "average_daily_deletions"
                    ],
                    "compliance_direction": compliance["summary"][
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get date range
            oldest_score = self.redis_client.zrange(
                "analytics:snapshots:index", 0, 0, withscores=True
            )
            newest_score = self.redis_client.zrange(

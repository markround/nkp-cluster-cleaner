...
class NotificationManager:
    ...
    def get_clusters_for_notification(
    ...
    ) -> Tuple[List[Tuple[Dict, float, datetime]], List[Tuple[Dict, float, datetime]]]:
        """
        ...
        clusters_to_delete, excluded_clusters = (
            self.cluster_manager.get_clusters_with_exclusions(namespace)
        )
                    ...
                    if creation_timestamp:
                        try:
                            ...
                            critical_clusters.append(
                                (cluster_info, 100.0, current_time)
                            )
                else:
    ...
    def get_cluster_notification_data(
        self, cluster_info: Dict, elapsed_percentage: float, expiry_time: datetime
    ) -> Dict:
        """
    ...
    def send_expiry_notification(
        self,
        backend: str,
        clusters: List[Tuple[Dict, float, datetime]],
        severity: str,
        threshold: int,
        **kwargs,
    ):
    ...
    def send_deletion_notification(
        self,
        backend: str,
        deleted_clusters: List[Dict[str, any]],
        severity: str = "info",
        **kwargs,
    ):
        """
